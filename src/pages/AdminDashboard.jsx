    ...
    const handleSearchAdd = async () => {
        ...
        try {
            ...
            if (data && data.length > 0) {
                const displayName = data[0].name || data[0].display_name.split(',')[0] || `Stop ${routeWaypoints.length + 1}`;
                setRouteWaypoints(prev => [...prev, { lat: parseFloat(data[0].lat), lng: parseFloat(data[0].lon), name: displayName }]);
                setSearchQuery('');
            } else {
                ...
                <motion.div className="glass-card" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}>
                    ...
                    <form onSubmit={handleRouteSubmit} className="flex flex-col gap-4 mb-8">
                        ...
                        {routeWaypoints.length > 0 && (
                            <div className="flex flex-col gap-2 p-2 border rounded" style={{ borderColor: 'var(--panel-border)', maxHeight: '150px', overflowY: 'auto', background: 'rgba(0,0,0,0.2)' }}>
                                <div className="flex justify-between items-center mb-1">
                                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{routeWaypoints.length} stops added</span>
                                    <button type="button" onClick={() => setRouteWaypoints([])} style={{ fontSize: '0.75rem', color: 'var(--accent-danger)', background: 'none', border: 'none', cursor: 'pointer' }}>Clear All</button>
                                </div>
                                {routeWaypoints.map((wp, idx) => (
                                    <div key={idx} className="flex justify-between items-center" style={{ fontSize: '0.85rem', background: 'var(--bg-input)', padding: '0.4rem 0.6rem', borderRadius: '4px' }}>
                                        <span style={{flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}}>{idx + 1}. {wp.name}</span>
                                        <button type="button" onClick={() => setRouteWaypoints(prev => prev.filter((_, i) => i !== idx))} style={{ color: 'var(--accent-danger)', background: 'none', border: 'none', cursor: 'pointer', paddingLeft: '0.5rem' }}><Trash2 size={14} /></button>
                                    </div>
                                ))}
                            </div>
                        )}
